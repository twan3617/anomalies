...
# find out thread_idx and n_threads from fig 7 of first paper commented.
    ...
    for thread_idx in range(1, n_threads):
        for i in prange(l):
            if ρ[0, i, 0] < ρ[thread_idx, i, 0] and ρ[thread_idx, i, 0] > nnthreshold:
                ρ[0, i, 0] = ρ[thread_idx, i, 0]
                I[0, i, 0] = I[thread_idx, i, 0]
            # left pearson correlation and left matrix profile indices
            if ρ[0, i, 1] < ρ[thread_idx, i, 1] and ρ[thread_idx, i, 1] > nnthreshold:
                ρ[0, i, 1] = ρ[thread_idx, i, 1]
                I[0, i, 1] = I[thread_idx, i, 1]
            # right pearson correlation and right matrix profile indices
            if ρ[0, i, 2] < ρ[thread_idx, i, 2] and ρ[thread_idx, i, 2] > nnthreshold:
                ρ[0, i, 2] = ρ[thread_idx, i, 2]
                I[0, i, 2] = I[thread_idx, i, 2]
...
@core.non_normalized(aamp)
def stump_threshold(T_A, m, T_B=None, ignore_trivial=True, normalize=True, p=2.0, nnthreshold=0):
    """
    Compute the z-normalized matrix profile
