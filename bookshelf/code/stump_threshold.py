...
# https://stumpy.readthedocs.io/en/latest/_modules/stumpy/floss.html#floss
# https://stumpy.readthedocs.io/en/latest/_modules/stumpy/stump.html

# what needs doing
# stumpy.stump - computation of matrix profile distances as well as the choice of nearest neigh
# all floss things which too update the nearest neighbour. this is complicated.
...
######################### begin copy from stumpy.stump sourcecode - all of it ############
...
def _stump(
    ...
    indices : numpy.ndarray
        ...
        column consists of the right matrix profile indices. #@@@yee
    ...
    @@@yee: update if smaller is the key in fig 7 - imbed threshold here 
    also line 12 in table 2 stomp algorithm
...
[docs]@core.non_normalized(aamp)
def stump(T_A, m, T_B=None, ignore_trivial=True, normalize=True, p=2.0, threshold=0):
    """
    ...
    @@@yee: https://escholarship.org/content/qt2x4419fp/qt2x4419fp_noSplash_63b12fd93e27078e9e7c61b5825dbae9.pdf
    something about using locality of search to compute dot prod of time series faster and hence distance between windows
    in particular, express euclidean distance as a decreasing function of pearson correlation coef between the two time series (pairs are determined by being the same time point)
    and exploit fastness idk
...
######################### end copy from stumpy.stump sourcecode ############

# def nearest_right_neigh(threshold_distance):
